...
// disabled policy can be undeletable until detached. Destroy must recover via the detach the API
// does allow, instead of leaving the resource stuck in state.
func TestShiftLeftPolicyDelete_LastActivePolicyRetriesAfterDetach(t *testing.T) {
	stub := &lastActivePolicyStub{allowDeleteWhenDetached: true}
	stub.start(t)
...
// lastActivePolicyStub always refuses the first DELETE the way the API does, and optionally refuses
// the detach too.
type lastActivePolicyStub struct {
	mu                      sync.Mutex
	allowDeleteWhenDetached bool
	detached                bool
	detaches                int
	deletes                 int
}
...
func (s *lastActivePolicyStub) start(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && r.URL.Path == "/api/shiftleft/projects/":
			...
			data := []map[string]any{}
			if start == 0 {
				data = append(data, map[string]any{"id": "proj-1", "name": "proj-1", "key": "proj-1"})
			}
		...
		case r.Method == http.MethodPut && strings.HasSuffix(r.URL.Path, "/projects/"):
			...
			if _, detaching := payload["projects_ids"]; detaching {
				s.mu.Lock()
				s.detaches++
				allowed := s.allowDeleteWhenDetached
				s.detached = allowed
				s.mu.Unlock()
				if !allowed {
