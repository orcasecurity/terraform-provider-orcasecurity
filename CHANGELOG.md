...
* **New Resource:** `orcasecurity_compliance_framework_selection` — enable/disable a compliance framework per `user`/`organization` scope. `scopes = []` is the explicit disable action. Destroy is state-only (built-in frameworks are not Terraform's to switch off).
...
* `orcasecurity_custom_compliance_framework`: nested sections (three levels), optional `description`/`tests`, `visibility`, `forced_cloud_vendors`, create-only `scope`, and extra test fields (`priority`, `control_unique_id`, `origin_framework_id`). `rule_id_in_framework` is now optional and derived as `<section-id>.<1-based index>` when omitted; on read it is the catalog `reference_id`.
