...
- `global_filter` (Boolean) Whether or not this is a business unit all users within your Orca org can use. If set to true, then it is accessible to all other users in your org. When omitted, Orca creates the business unit as global, so this is read back as `true`.
