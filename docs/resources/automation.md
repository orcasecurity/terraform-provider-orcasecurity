...
- `exclude_prefix` (List of String) Exclude values that start with any of the specified prefixes.
- `excludes` (List of String) When `excludes` is used, the automation applies to the negation of the specified field values.
- `includes` (List of String) When `includes` is used, the automation applies to the specified field values.
- `prefix` (List of String) Match values that start with any of the specified prefixes.
- `range` (Attributes) Range-based filtering for numeric fields. Use for fields like `state.orca_score`. (see [below for nested schema](#nestedatt--query--filter--range))

<a id="nestedatt--query--filter--range"></a>
### Nested Schema for `query.filter.range`
