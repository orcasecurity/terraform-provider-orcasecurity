...
!> **Adopt semantics:** Adopting a group that already exists in Orca — as opposed to a fresh `integrate` of a not-yet-integrated group — refuses unless `adopt_existing = true`; `destroy` de-integrates and drops whatever it holds (repos, policies, settings). Prefer `terraform import` to avoid a takeover write.
