...
- `version_agnostic_display_name` (String) Exact match on `version_agnostic_display_name` (the family name without version, e.g. `PCI DSS`). Use this to pin a framework family across upgrades. Custom frameworks have a null `version_agnostic_display_name` and never match.
